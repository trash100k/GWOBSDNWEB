...
- **Codrops — "Building a Fully-Featured 3D World in the Browser with Blender and Three.js"**, 8 Apr 2025 — authoring a camera path by exporting a curve from Blender to JSON and driving it with `curve.getPoint()` + lerp on scroll in the r3f `useFrame` loop; the modern "scroll amount → curve param → damped follow" pattern, and the case for arc-length vs raw `getPoint`. https://tympanus.net/codrops/2025/04/08/3d-world-in-the-browser-with-blender-and-three-js/
