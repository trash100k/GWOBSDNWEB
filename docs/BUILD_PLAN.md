...
- **DONE this session (all on the branch, build-green, 0 console errors @ 393×852 + 1440×900):**
  ...
  - **Mandala** REBUILT geometric (real `<circle>` rings + N-fold `rotate()` motifs) AND **re-framed** (centred, whole, no scroll scale/spin). **SPLIT BUG FIXED:** the `.m-spin` motif bands rotated around the viewBox CORNER (`view-box`+`50% 50%` → user `(500,500)` in Chrome), flinging petals/word-rim to the bottom-right while the skeleton stayed centred → now `transform-box:fill-box; transform-origin:center` (pivot on own symmetric bbox).
  - **Finale = the mandala is the END:** journey now SETTLES ONTO the mandala (final resting image — its rim reads GAELWORX, so it IS the forge-seal); GAELWORX wordmark + `Start the Forge` CTA seat at its eye and HOLD to the end. Folded out the solutions/four-forges/seed tail; finale scroll weight `2.7→1.95`. CTA → `/contact` preserved.
  ...
  - **Mandala QA loop** (`scripts/mandala-qa.mjs`, `npm run qa:mandala`): geometric/DOM, deterministic (reduced-motion → Lenis off → exact scroll). Asserts concentric (getBBox, 0.00u), designed radii, strictly nested, in-frame, centred, **spin-group pivot** (forces 90° rotation — caught the split the ring-only checks missed), **end-state** (mandala held + CTA live at scroll bottom), 0 console errors @ 393×852 + 1440×900. Exits non-zero. GREEN.
